...
export default function OrdersUdapte() {
  const { user } = useAuth();
          ...
          {filtered.map(order => {
            ...
            return (
              <div key={order.id} className={`order-card card ${order.status}`}>
                <div className="order-card-header" onClick={() => setExpanded(isOpen ? null : order.id)}>
                  <div className="order-id-col">
                    <span className="order-id">#{String(order.id).slice(-6).toUpperCase()}</span>
                  ...
                  <div className="order-total-col">
                    ...
                    <span className="order-items-count">{items.length} producto{items.length !== 1 ? 's' : ''}</span>
                  </div>
                  <div className="order-status-col">
                    <span className={`status-badge status-${order.status}`}>{status.emoji} {status.label}</span>
                  </div>
                        ...
                        <div className="order-items-list">
                          {items.map((item, i) => (
                            <div key={i} className="order-item-row">
                              <img src={item.image || 'https://placehold.co/48x48?text=?'} alt={item.name} className="order-item-img" />
                              <div className="order-item-info">
                                <span className="order-item-name">{item.name}</span>
                                <span className="order-item-qty">{item.quantity}x — {formatPrice(item.price)}</span>
                              </div>
                              <span className="order-item-subtotal">{formatPrice(item.price * item.quantity)}</span>
                            </div>
                          ))}
                        </div>
                          ...
                          <div className="status-buttons">
                            {STATUS_OPTIONS.map(s => (
                              <button key={s.value}
                                ...
                                {updating === order.id && order.status !== s.value ? '...' : `${s.emoji} ${s.label}`}
                              </button>
                            ))}
                          </div>
                        </div>
